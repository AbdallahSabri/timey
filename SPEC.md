...
**`/reset-password` is reachable in every auth state, and gated by a marker cookie.** Middleware's public early return is the only exit before the profile lookup, so any other placement bounces a limbo invitee to `/onboarding` before they can set a password — and an invited employee who never onboarded is precisely the person a recovery link has to work for. Passing that early return would otherwise hand an ordinary signed-in user a working change-password form, so `/auth/reset` sets a short-lived `httpOnly` marker immediately before redirecting.

**The marker's value is the user id `verifyOtp` returned, and that is load-bearing rather than incidental.** A browser-scoped marker would hand the form to whoever signs in next on a shared machine: Alice opens her link and walks away, Bob signs in within the marker's lifetime, and the form would change *Bob's* password on Bob's session with no old password asked for. So the gate refuses unless the marker names the user the request is authenticated as.

**The gate lives in `updatePassword`, not only on the page.** A server action is a network-reachable endpoint, so a check that only decides whether a page renders is a check a crafted POST walks past — the same reasoning §4.2.2 applies to admin routes and §8.1.1 applies to `create_company()`. The page's check is the convenience; the action's is the enforcement, and both read one helper so they cannot drift.
