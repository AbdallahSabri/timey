...
- **A single-admin company has no direct path to delete a closed entry or create a backdated one — only amend.** `admin_edit_entry` (§7.4's direct-edit path) covers amendment only; delete and create still require the request queue, which a lone admin can never approve for themselves. Real, reachable gap for the most common company size — tracked as `BLOCKERS.md` N-8, not closed here. The workaround today is temporarily promoting a second admin.
