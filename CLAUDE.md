...
| `src/lib/auth/**` | `implement-logic` | Auth-flow state a `'use server'` module can't publish — the `/reset-password` recovery cookie, shared by a route handler, a page and an action |
